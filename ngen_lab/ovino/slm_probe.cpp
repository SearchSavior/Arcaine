...
int main(int argc, char** argv) {
    const char* spv_path = argc > 1 ? argv[1] : "ngen_lab/ovino/slm_probe.spv";
    ZE_CHECK(zeInit(0));
    uint32_t ndrv = 0;
    ...
    ze_kernel_desc_t kdesc{ZE_STRUCTURE_TYPE_KERNEL_DESC, nullptr, 0, "slm_probe"};
    ...
    ZE_CHECK(zeMemAllocDevice(context, &dd, 4, 64, device, &buf));
    ...
    ZE_CHECK(zeCommandListAppendMemoryCopy(list, out.data(), buf, 4, nullptr, 0, nullptr));
    ZE_CHECK(zeCommandListHostSynchronize(list, UINT64_MAX));
    for (int i = 0; i < 32; ++i)
