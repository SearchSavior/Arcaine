// Diagnostic host for diag3: grid {512,1,1} local {16,1,1}.
...
int main(int argc, char** argv) {
    const char* spv_path = argc > 1 ? argv[1] : "ngen_lab/ovino/diag3.spv";
    ZE_CHECK(zeInit(0));
    uint32_t ndrv = 0;
    ...
    ze_kernel_desc_t kdesc{ZE_STRUCTURE_TYPE_KERNEL_DESC, nullptr, 0, "diag3"};
    ...
    ZE_CHECK(zeMemAllocDevice(context, &dd, 512 * 4 * 4, 64, device, &out));
    ZE_CHECK(zeKernelSetArgumentValue(kernel, 0, sizeof(void*), &out));

    ze_group_count_t gc{16, 1, 1};
    ...
    std::vector<uint32_t> outh(512 * 4);
    ZE_CHECK(zeCommandListAppendMemoryCopy(list, outh.data(), out, 512 * 4 * 4, nullptr, 0, nullptr));
    ZE_CHECK(zeCommandListHostSynchronize(list, UINT64_MAX));

    std::printf("glid0 | gid2 | lid | sg\n");
