...
int main(int argc, char** argv) {
    ...
    std::string spv_path = "ngen_lab/ovino/gdn_seq_ref_x.spv";
    int iters = 5;
    ...
    for (int T : tokens) {
        ...
        if (T == 8) {
            ...
            std::printf("  [dump] state h=1 k=0..3 v=0: ref vs gpu\n");
            for (int k = 0; k < 4; ++k)
                std::printf("    h1k%d ref=%9.5f gpu=%9.5f\n", k, s1[128*128 + k*128], s_gpu[128*128 + k*128]);
        }
