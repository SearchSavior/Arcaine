...
// Arcaine addition: expose the Qwen2 custom pre-tokenizer directly so the
// tokenizer can route to it without depending on regex-string matching in the
// dispatcher above (the on-disk tokenizer.json uses the "(?i:...)" form, which
// the dispatcher would mis-route to the Llama-3 family).
std::vector<size_t> unicode_regex_split_custom_qwen2(const std::string & text, const std::vector<size_t> & offsets);
