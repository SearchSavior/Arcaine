...
bool select_kernel(const std::string& kernel) {
    bool dpas;
    if (kernel == "onednn" || kernel == "default") {
        ...
        throw std::runtime_error(
            "unknown kernel '" + kernel + "' (use: onednn, xe2-dpas)");
    }
    if (::setenv("ARCAINE_QWEN35_NVFP4_DPAS", dpas ? "1" : "0", 1) != 0)
...
void usage(const char* program) {
    std::fprintf(stderr,
        ...
        "  --kernels <csv>   kernels: onednn,xe2-dpas       (default: both)\n"
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        if (token_counts.empty() || kernels.empty() || warmup < 0 || runs <= 0 ||
            layer < 0 || layer > 55)
        ...
        std::printf("[bench] matrix: kernels={%s} p={%s} | warmup=%d runs=%d | cells=%zu\n",
                    kernels_csv.c_str(), p_csv.c_str(), warmup, runs,
                    kernels.size() * token_counts.size());
        std::printf("[bench] layer=%d model=%s\n", layer, model.c_str());
        if (md)
            std::printf("| kernel | p | runs | mean ms | sd ms | tokens/s |\n"
                        "|---|---:|---:|---:|---:|---:|\n");

        for (const std::string& kernel : kernels) {
            bool dpas = select_kernel(kernel);
            for (int tokens : token_counts) {
                auto run = [&] {
                    if (dpas) {
