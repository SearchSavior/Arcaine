// gemma4_unified — model-owned inference benchmark (llama-bench style).
...
// depths. Registered with the central arcaine_mbench dispatcher as
// "gemma4_unified" (matches config.json::model_type).
//
// The dispatcher (apps/model_bench/main.cpp) resolves config.json::model_type and
// forwards argv; this run() owns all gemma4 benchmark logic.
//
...
static const char* USAGE =
    "Usage: arcaine_mbench --model <dir> [options]   (gemma4_unified)\n"
...
static int run(int argc, char* argv[]) {
    ...
    double t0 = now_ms();
    Gemma4Model model(model_dir, max_seq);
...
REGISTER_MODEL_BENCH("gemma4_unified", "Gemma4 Unified AR (PP/TG KV-depth throughput)", run)
