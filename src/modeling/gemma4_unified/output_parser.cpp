...
std::optional<inference::ParsedToolCall> parse_one_tool_call(const std::string& body, size_t index) {
    ...
    return inference::ParsedToolCall{
...
Gemma4BoundaryParser::Gemma4BoundaryParser(const std::string& model_dir) {
...
Gemma4BoundaryCounts
Gemma4BoundaryParser::count(const std::vector<int>& token_ids) const {
    Gemma4BoundaryCounts counts;
    size_t i = 0;
...
bool Gemma4BoundaryParser::has_thought_markers() const {
    return !thought_start_ids_.empty() && thought_end_id_ >= 0;
}

bool Gemma4BoundaryParser::matches_at(const std::vector<int>& token_ids, size_t pos,
...
const std::vector<int>* Gemma4BoundaryParser::matching_response_channel_at(
...
bool Gemma4BoundaryParser::is_ignored_response_id(int token_id) const {
