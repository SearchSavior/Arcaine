...
// ---------------------------------------------------------------------------
// Sharded safetensors: reads model.safetensors.index.json and maps each tensor
// name to the shard that holds it.
// ---------------------------------------------------------------------------
class TensorSource {
public:
...
public:
    explicit ShardedSafetensors(const std::string& model_dir) {
        ...
        if (!f) {
            // Single-file checkpoint (e.g. the int4 W4A16 build): no shard index,
            // just model.safetensors. Map every tensor name to that one shard.
    ...
    bool has(const std::string& name) const override { return name_to_shard_.count(name) > 0; }
...
// Set DIFF_LOAD_TRACE=1 for per-tensor stall debugging.
...
static GpuBuffer<bf16> upload(const TensorView& tv, sycl::queue& q,
                              const char* name = "?") {
    size_t n = tv.numel();
...
static GpuBuffer<uint8_t> upload_u8(const TensorView& tv, sycl::queue& q,
                                    const char* name = "?") {
...
static float scalar_f32(const TensorView& tv, const char* name = "?") {
...
static GpuBuffer<uint8_t> upload_nvfp4_scales_transposed(
...
static Nvfp4Linear upload_nvfp4_linear(const TensorSource& sf,
...
static Nvfp4Linear upload_nvfp4_linear_pair(const TensorSource& sf,
