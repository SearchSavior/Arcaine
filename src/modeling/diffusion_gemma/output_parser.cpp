...
std::string collapse_duplicate_braces_outside_strings(const std::string& text) {
    ...
    for (size_t i = 0; i < text.size();) {
        ...
        if (!in_string && (text[i] == '{' || text[i] == '}')) {
            char brace = text[i];
            out.push_back(brace);
            while (i < text.size() && text[i] == brace) ++i;
            continue;
        }
...
parse_one_tool_call(const std::string& body, size_t index) {
    ...
    return arcaine::inference::ParsedToolCall{
...
void strip_gemma_channels(std::string& content) {
    ...
    while (true) {
        ...
        if (e == std::string::npos ||
            ...
            if (next_channel == std::string::npos) { content.erase(s); break; }
    ...
    while (true) {
        ...
        if (label_end == std::string::npos) { content.erase(s, channel_start.size()); search = s; }
        else { content.erase(s, label_end + 1 - s); search = s; }
    }
...
DiffusionGemmaAssistantOutput parse_assistant_output(const std::string& raw_text) {
    DiffusionGemmaAssistantOutput out;
...
DiffusionGemmaBoundaryParser::DiffusionGemmaBoundaryParser(const std::string& model_dir) {
...
DiffusionGemmaBoundaryCounts
DiffusionGemmaBoundaryParser::count(const std::vector<int>& token_ids) const {
    DiffusionGemmaBoundaryCounts counts;
    size_t i = 0;
    while (i < token_ids.size()) {
        if (has_thought_markers() && matches_at(token_ids, i, thought_start_ids_)) {
            ...
            if (i < token_ids.size() && token_ids[i] == thought_end_id_) ++i;
            continue;
        }
        ...
        if (!is_ignored_response_id(token_ids[i])) ++counts.response_tokens;
        ++i;
    }
    return counts;
}

bool DiffusionGemmaBoundaryParser::has_thought_markers() const {
    return !thought_start_ids_.empty() && thought_end_id_ >= 0;
}
bool DiffusionGemmaBoundaryParser::matches_at(const std::vector<int>& token_ids, size_t pos,
                                              const std::vector<int>& pattern) const {
    if (pattern.empty() || pos + pattern.size() > token_ids.size()) return false;
    for (size_t i = 0; i < pattern.size(); ++i)
        if (token_ids[pos + i] != pattern[i]) return false;
    return true;
}
const std::vector<int>* DiffusionGemmaBoundaryParser::matching_response_channel_at(
    const std::vector<int>& token_ids, size_t pos) const {
    for (const auto& pattern : response_channel_start_ids_)
        if (matches_at(token_ids, pos, pattern)) return &pattern;
    return nullptr;
}
bool DiffusionGemmaBoundaryParser::is_ignored_response_id(int token_id) const {
    return std::binary_search(ignored_response_ids_.begin(), ignored_response_ids_.end(), token_id);
}
