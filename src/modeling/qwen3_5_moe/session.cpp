...
QwenMoeSession::QwenMoeSession(QwenMoeService& service,
                               const arcaine::inference::SessionCreateRequest& req)
    : service_(service), cancel_(req.cancellation) {}

arcaine::inference::GenerationResult QwenMoeSession::generate(
    ...
    QwenMoeInvocation inv = map_request(request, service_.model(),
    ...
    QwenMoeCache cache(service_.model());
    cache.reset();

    QwenMoeDecodeResult r = run_decode_loop(service_.model(), inv,
                                            service_.tokenizer(),
                                            sink, cancel_);
