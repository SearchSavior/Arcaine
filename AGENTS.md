...
## Model implementation isolation (enforced)

Every supported model architecture (gemma4_unified, qwen3_5, qwen3_5_moe, diffusion_gemma) is a self-contained module under `src/modeling/<model>/`. Each model owns its complete inference path: registration, loading, request resolution, session state, generation orchestration, sampling, cache, placement, expert execution, output parsing, kernels, and benchmarks. The HTTP server and other applications interact with models only through the stable inference contracts in `src/inference/` (`ModelService`, `InferenceSession`, `ModelRegistry`, `GenerationRequest`, `GenerationEvent`, `GenerationResult`).

- A model module MUST NOT include another model module's headers. Shared tensor primitives (rope, rms_norm, layer_norm, elementwise, scatter, embedding, attention_mask, attention_batched, attention_layout) are COPIED into each consuming model's `kernels/`; there is no shared `common/kernels/` or `common/layers/`.
- `src/runtime/` and `src/inference/contracts/` MUST NOT include `modeling/`. (`src/inference/registry/builtin_models.cpp` is the one sanctioned composition root that includes each model's `registration.hpp` to wire the registry.)
- Applications (`src/apps/server/`, `src/apps/model_bench/`, `src/apps/kernel_bench/`) MUST NOT branch on model family. The server is model-agnostic (one `ModelService` → `create_session` → `generate(sink)` path). Model-owned dev tools (`src/apps/diffusion/`) and per-model benchmarks (`modeling/<m>/benchmarks/`) construct their model's concrete engine directly and are exempt from the contract surface.
- The legacy `Model` interface, `ModelRegistry::instance()`, and per-model `register.*` are retired. Model interfaces (`ForwardInput`/`PreparedInput`/`ModelInfo`) are owned per-model where used.
- Enforce with: `grep -R "modeling/" src/apps/server src/inference/contracts src/runtime` (empty); cross-model greps between `src/modeling/<m>/` dirs (empty); `grep -R "common/model_interface\|common/registry\|: public Model" src/` (empty).
- `ARCAINE_MODELS` selects which models compile into a build; a disabled model's sources are not compiled, and loading it at runtime yields a clear error listing registered model types.
